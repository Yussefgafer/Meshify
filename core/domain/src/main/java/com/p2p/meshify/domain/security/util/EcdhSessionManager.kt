...
class EcdhSessionManager {
    ...
    fun finalizeSessionKey(
        ...
        val myEphemeralPrivKey = try {
            val keyFactory = KeyFactory.getInstance("EC")
            val myPrivKeySpec = PKCS8EncodedKeySpec(myEphemeralPrivateKey)
        ...
        // Build HKDF salt: nonce_initiator || nonce_responder (concatenated nonces, 32 bytes total)
        // CRITICAL: Salt must be initiator_nonce || responder_nonce for both parties
        // CRITICAL: Both parties MUST use same order: initiator_nonce || responder_nonce
        // In finalizeSessionKey: myNonce=initiator, peerNonce=responder
        val hkdfSalt = myNonce + peerNonce  // initiator || responder
