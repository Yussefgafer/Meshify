...
- [Chore] Removed dead BleGattClient.disconnect(peerId) method — repo-wide grep confirmed zero callers (BleTransportImpl and BleTransportTestAdapter both rely on cleanup() instead); the method's body duplicated failAndRelease() logic already reachable via cleanup() so no semantic is lost
- [Fix] Onboarding BT permission card is now correctly gated to Android 12+ (API 31+) — the card in WelcomeScreen previously requested BLUETOOTH_SCAN / BLUETOOTH_CONNECT / BLUETOOTH_ADVERTISE unconditionally, but those are only runtime permissions on API 31+; on API ≤30 they are install-time perms (declared in core/network/AndroidManifest.xml with `maxSdkVersion="30"` for legacy BLUETOOTH / BLUETOOTH_ADMIN) and are granted automatically without a card, so a pre-12 user would see a card they could not meaningfully act on. MainActivity.checkAndRequestPermissions already had the same gate; WelcomeScreen now matches
- [Fix] BLE start no longer leaks scope + connectionPool on a failed GATT server service-add — BleTransportImpl.start() allocates a fresh CoroutineScope and BleConnectionPool before awaiting the GATT server's onServiceAdded callback; if the await returns false, stop()'s `!isStarted` guard short-circuits and the resources leak until process death. New private `tearDownPartialStart()` cancels and nulls the scope, clears and nulls the connection pool, and stops the GATT server; called from the `!serverReady` branch, the outer `catch (e: Exception)` branch, and a `catch (e: CancellationException)` rethrow branch so even a mid-start coroutine cancellation releases the resources before the exception is re-propagated
...
- [Fix] BLE toggle no longer lies when runtime perms are denied — SettingsViewModel.setBleEnabled still writes the preference unconditionally, but MeshifyApp's `bleEnabled` collect now checks BLUETOOTH_SCAN / BLUETOOTH_CONNECT / BLUETOOTH_ADVERTISE (only on API ≥ 31; pre-12 has install-time perms) before instantiating BleTransportImpl, logs the rejection, and reverts the setting to false; without the guard BleGattServer.startServer / BleScanner would have SecurityException'd silently and the toggle would have reported "on" while the transport was never started
- [Chore] Stale `transportManager` parameter now passed to SettingsViewModel — Hilt's MainActivity factory for the settings VM was passing only the repository, so `transportManager.bleRuntimeActive.onEach { ... }` (which mirrors real BLE runtime state into the UI to feed the BleStatusBottomSheet) would have crashed the moment the VM was created on the next launch; the second constructor arg was added in an earlier fix but the factory call site was missed. The first `.compileDebugKotlin` after the S1 guard landed surfaced the staleness and forced the fix in the same batch
- [Fix] BLE now self-heals a Quick-Settings Bluetooth toggle — BleTransportImpl watches BluetoothAdapter.ACTION_STATE_CHANGED via a BroadcastReceiver registered after a successful start. On STATE_TURNING_OFF it performs an inline teardown (server, advertiser, scanner, client, pool, peer maps) while keeping the receiver alive; on STATE_ON it re-issues start() when BLE was still wanted, so toggling BT off and on from the system shade no longer leaves the transport half-live (every operation failing silently) with no recovery. The monitor runs on its own scope (separate from the transport scope) so a failed re-start() cannot strand the receiver, and stop() now guards on `bleWantedOn || isStarted` so an explicit in-app BLE-off after a system-shade teardown still clears bleWantedOn and unregisters the receiver — never self-healing against the user's intent
