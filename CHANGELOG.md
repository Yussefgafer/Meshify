...
- [Fix] `advanceTrigger` moved from plain `remember` to `rememberSaveable` — rotating the device inside the 200ms auto-advance window no longer resets the trigger to 0 and strands the flow on the current card
